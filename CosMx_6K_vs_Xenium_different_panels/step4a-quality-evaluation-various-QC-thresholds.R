...
for (i in 1:nrow(sample_list)) {
	...
	for (threshold in seq(0, 200, 10)) {
		if (threshold == 0) {
			smi <- readRDS(paste0("comparison/", sample_id, "_smi_so.RDS"))
			xen <- readRDS(paste0("comparison/", sample_id, "_xen_so.RDS"))
		} else {
			smi <- readRDS(paste0("comparison_multi_thr/", sample_id, "_smi_so_qcnorm_thr", as.character(threshold), ".RDS"))
			xen <- readRDS(paste0("comparison_multi_thr/", sample_id, "_xen_so_qcnorm_thr", as.character(threshold), ".RDS"))
		}
		...
		results_uncorrected_overlap[[k]] <- data.table(sample_id = rep(sample_id, ncol(smi_counts_overlap)), tech = rep("CosMx", ncol(smi_counts_overlap)), threshold = threshold, cell_id = colnames(smi_counts_overlap), n_cell = rep(ncol(smi_counts_overlap), ncol(smi_counts_overlap)), n_gene = rep(nrow(smi_counts_overlap), ncol(smi_counts_overlap)), total_count = as.integer(total_count), diversity = as.integer(diversity))
		k <- k + 1
		...
		results_uncorrected_overlap[[k]] <- data.table(sample_id = rep(sample_id, ncol(xen_counts_overlap)), tech = rep("Xenium", ncol(xen_counts_overlap)), threshold = threshold, cell_id = colnames(xen_counts_overlap), n_cell = rep(ncol(xen_counts_overlap), ncol(xen_counts_overlap)), n_gene = rep(nrow(xen_counts_overlap), ncol(xen_counts_overlap)), total_count = as.integer(total_count), diversity = as.integer(diversity))
		k <- k + 1
...
fwrite(results_uncorrected_full, "comparison_multi_thr/results_uncorrected_full.txt", sep = "\t")
fwrite(results_uncorrected_overlap, "comparison_multi_thr/results_uncorrected_overlap.txt", sep = "\t")

##### Pull cell areas from before QC data
...
for (i in 1:nrow(sample_list)) {
	...
	results_areas[[i]] <- data.table(sample_id = rep(sample_id, ncol(smi) + ncol(xen)), tech = c(rep("CosMx", ncol(smi)), rep("Xenium", ncol(xen))), cell_id = c(colnames(smi), colnames(xen)), cell_area = c(smi$cell_area_um2, xen$cell_area))
}
results_areas <- rbindlist(results_areas)
fwrite(results_areas, "comparison_multi_thr/results_areas.txt", sep = "\t")

##### Standardize quality metrics by cell area

library(data.table)

results_uncorrected_full <- fread("comparison_multi_thr/results_uncorrected_full.txt")
results_uncorrected_overlap <- fread("comparison_multi_thr/results_uncorrected_overlap.txt")
results_areas <- fread("comparison_multi_thr/results_areas.txt")
...
fwrite(results_uncorrected_full, "comparison_multi_thr/results_combined_full_panel.txt", sep = "\t")
fwrite(results_uncorrected_overlap, "comparison_multi_thr/results_combined_overlap_panel.txt", sep = "\t")
