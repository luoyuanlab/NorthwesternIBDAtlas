...
for (i in 1:nrow(sample_list)) {
	...
	so <- readRDS(paste0("~/", as.character(sample_id), "_smi_so_norm.RDS"))
	use_cells <- colnames(so)[so$nCount_Nanostring >0]
	so <- so[, use_cells]
	so@images$smi <- NULL
...
so <- merge(x = so_list[[1]], y = c(subsetList(so_list, seq(2, length(so_list)))), add.cell.ids = as.character(1:4), project = "CosMx")
...
fwrite(category_DGE, "~/cosmx_merged_category_DGE.txt", sep = "\t")
...
for (i in 1:nrow(sample_list)) {
	# i <- 1
	sample_id <- sample_list$sample_id[i]
	so <- readRDS(paste0("~/", as.character(sample_id), "_xen_so_norm.RDS"))
	so@images$fov <- NULL
	use_cells <- colnames(so)[so$nCount_Xenium >0]
	so <- so[, use_cells]
...
so <- merge(x = so_list[[1]], y = c(subsetList(so_list, seq(2, length(so_list)))), add.cell.ids = as.character(1:4), project = "Xenium")
so <- PrepSCTFindMarkers(object = so)
VariableFeatures(so) <- rownames(so) # set all the genes to be variable because we want to use all the genes
so <- RunPCA(so, assay = "SCT", verbose = FALSE)
so <- RunUMAP(so, reduction = "pca", dims = 1:40, verbose = FALSE)
...
Idents(so) <- annotations
category_DGE <- FindAllMarkers(so, assay = "SCT")
fwrite(category_DGE, "~/xenium_merged_category_DGE.txt", sep = "\t")
...
for (i in 1:nrow(sample_list)) {
  # i <- 1
  sample_id <- sample_list$sample_id[i]
  so <- readRDS(paste0("~/", as.character(sample_id), "_xen_so_unQVfiltered_norm.RDS"))
  use_cells <- colnames(so)[so$nCount_Xenium >0]
  so <- so[, use_cells]
  so@meta.data$sample_id <- sample_id
  so@meta.data$block <- sample_list$block[i]
  ...
  #need to remove all the unassigned and negprobes
  bad_pat <- "(?i)^(NegControl(Codeword|Probe)|UnassignedCodeword)"  # case-insensitive
  keep <- !grepl(bad_pat, rownames(so))
  so <- subset(so, features = rownames(so)[keep])
  so=so[,unname(which(colSums(GetAssayData(so))!=0))]
...
Idents(so) <- annotations
category_DGE <- FindAllMarkers(so, assay = "SCT")
fwrite(category_DGE, "~/xenium_unQVfiltered_merged_category_DGE.txt", sep = "\t")