# cd /share/fsmresfiles/UC/AtoMx/UC_serial_cuts/6000plex
# conda activate R4
# R
...
for (i in 1:nrow(sample_list)) {
  ...
  print(paste0("working on ",sample_list$xenium_377_with_seg[i]))
  #panel <- list(smi = list(), xen = list())
  
  ### Xenium
  data_path <- paste0("~/", sample_list$xenium_377_with_seg[i])
  ...
  #we don't use the default criteria QV >=20 to filter it 
  parquet_file_assign <- parquet_file %>% filter(cell_id != "UNASSIGNED") %>% filter(is_gene == TRUE) #only for 5k is_gene == TRUE
  ...
  saveRDS(seurat_obj_unfiltered, paste0("~/", sample_id, "_xen_377_with_seg_so_unQVfiltered.RDS"))
}
...
for (i in 1:nrow(sample_list)) {
  #i <- 1
  sample_id <- sample_list$sample_id[i]
  print(paste0("working on ",sample_list$xenium_377_no_seg[i]))
  #panel <- list(smi = list(), xen = list())
  
  ### Xenium
  data_path <- paste0("~/", sample_list$xenium_377_no_seg[i])
  parquet_path <- file.path(data_path, "transcripts.parquet")
  ...
  saveRDS(seurat_obj_unfiltered, paste0("~/", sample_id, "_xen_377_no_seg_so_unQVfiltered.RDS"))
}
...
for (i in 1:nrow(sample_list)) {
  #i <- 1
  sample_id <- sample_list$sample_id[i]
  print(paste0("working on ",sample_list$xenium_folder_colon[i]))
  #panel <- list(smi = list(), xen = list())
  
  ### Xenium
  data_path <- paste0("~/", sample_list$xenium_folder_colon[i])
  parquet_path <- file.path(data_path, "transcripts.parquet")
  ...
  saveRDS(seurat_obj_unfiltered, paste0("~/", sample_id, "_xen_colon_with_seg_so_unQVfiltered.RDS"))
}