...
sc <- readRDS("CD_sc.RDS")
cell_map <- readRDS("CD_cell_map.RDS")
ref_profile <- readRDS("CD_minor_profile.RDS")
...
### Crohn's disease reference -- https://pubmed.ncbi.nlm.nih.gov/34497389/
sc_imm <- readRDS("CD_sc_imm.RDS")
cell_map_imm <- readRDS("CD_cell_map_imm.RDS")
sc_epi <- readRDS("CD_sc_epi.RDS")
cell_map_epi <- readRDS("CD_cell_map_epi.RDS")
sc_others <- readRDS("CD_sc_others.RDS")
cell_map_others <- readRDS("CD_cell_map_others.RDS")
gc()
...
fwrite(summary, "comparison/CosMx16_CD_ct_prop_summary.txt", sep = "\t")
