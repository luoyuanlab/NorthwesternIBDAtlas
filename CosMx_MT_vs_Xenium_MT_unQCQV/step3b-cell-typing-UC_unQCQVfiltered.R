...
sc <- readRDS("~/scRNA-seq/UC/sc.RDS")
cell_map <- readRDS("~/scRNA-seq/UC/cell_map.RDS")
ref_profile <- readRDS("~/scRNA-seq/UC/minor_profile.RDS")
...
for (i in 1:nrow(sample_list)) {
	...
	xen <- readRDS(paste0("~/", as.character(sample_id), "_xen_so_norm.RDS"))
	xen_unfiltered <- readRDS(paste0("~/", as.character(sample_id), "_xen_so_unQVfiltered_norm.RDS"))
	...
	# ### CosMx (RNA only)
	# Find anchors
	anchors <- FindTransferAnchors(reference = sc, query = smi,verbose = T)
	# Pull predictions
	predictions <- TransferData(anchorset = anchors, refdata = sc$celltype_minor, weight.reduction = "pcaproject",verbose = T)
	...
	### Xenium
	# Find anchors
	anchors <- FindTransferAnchors(reference = sc, query =xen,verbose = T)
	# Pull predictions
	predictions <-TransferData(anchorset = anchors, refdata = sc$celltype_minor, weight.reduction = "pcaproject",verbose = T)
	# Add predictions to seurat
	...
	# Find anchors 
	anchors <- FindTransferAnchors(reference = sc, query =xen_unfiltered,verbose = T)
	# Pull predictions
	predictions <-TransferData(anchorset = anchors, refdata = sc$celltype_minor, weight.reduction = "pcaproject",verbose = T)
	# Add predictions to seurat
	...
	saveRDS(xen_unfiltered, paste0("~/", sample_id, "_xen_so_unQVfiltered_norm.RDS"))
...
### Ulcerative colitis reference -- https://pubmed.ncbi.nlm.nih.gov/31348891/
sc_imm <- readRDS("~/scRNA-seq/UC/sc_imm.RDS")
cell_map_imm <- readRDS("~/scRNA-seq/UC/cell_map_imm.RDS")

sc_epi <- readRDS("~/scRNA-seq/UC/sc_epi.RDS")
cell_map_epi <- readRDS("~/scRNA-seq/UC/cell_map_epi.RDS")

sc_others <- readRDS("~/scRNA-seq/UC/sc_others.RDS")
cell_map_others <- readRDS("~/scRNA-seq/UC/cell_map_others.RDS")
gc()
...
for (i in 1:nrow(sample_list)) {
  ...
  saveRDS(smi, paste0("~/", as.character(sample_id), "_smi_so_norm.RDS"))
  gc()
}
...
fwrite(summary, "~/CosMx16_UC_ct_prop_summary_new.txt", sep = "\t")
